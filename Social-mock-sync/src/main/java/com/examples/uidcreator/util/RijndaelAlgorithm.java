...
public class RijndaelAlgorithm // implicit no-argument constructor
{
...
//        if (DEBUG && debuglevel > 6) {
//            System.out.println("CT=" + formatLong(result));
//            System.out.println();
//        }
...
//        if (DEBUG && debuglevel > 6) {
//            System.out.println("PT=" + formatLong(result));
//            System.out.println();
//        }
//        if (DEBUG) trace(OUT, "blockDecrypt()");
    ...
    /**
     * A basic symmetric encryption/decryption validate.
     */
    ...
    public static byte[] pad(byte[] bytes, int offsite, int len) throws UnsupportedEncodingException {
        if (len >= BLOCK_SIZE) return bytes;

        final byte[] res = new byte[BLOCK_SIZE];
        System.arraycopy(bytes, offsite, res, 0, len);
        int addLen = BLOCK_SIZE - (len);
    ...
    public static byte[] unPad(byte[] outBuff) throws UnsupportedEncodingException {
        int size = outBuff.length;
        ...
        for (int i = 0; i < padValue; i++) {
            if (outBuff[--size] != padValue) return outBuff;
        }

        byte[] res = new byte[size];
        System.arraycopy(outBuff, 0, res, 0, size);
        return res;
    }
...
//            if (DEBUG && debuglevel > 6) System.out.println("CT" + r + "=" + formatLong(t));
        }
...
//        if (DEBUG) trace(IN, "blockDecrypt(" + in + ", " + inOffset + ", " + sessionKey + ", " + blockSize + ")");
        ...
        for (int r = 1; r < ROUNDS; r++) {          // apply round transforms
            ...
            System.arraycopy(a, 0, t, 0, BC);
//            if (DEBUG && debuglevel > 6) System.out.println("PT" + r + "=" + formatLong(t));
        }
        for (i = 0; i < BC; i++) {                   // last round is special
            tt = Kd[ROUNDS][i];
...
//        if (DEBUG) trace(OUT, "blockDecrypt()");
    ...
    /**
     * A basic symmetric encryption/decryption validate for a given key size.
     */
...
//                System.out.println("KEY=" + formatLong(kb));
//                System.out.println();
//            }
...
//                System.out.println();
//                System.out.println("PT=" + formatLong(pt));
//            }
...
//                System.out.println();
//                System.out.println("CT=" + formatLong(ct));
//            }
...
//                debug("Exception encountered during self-validate: " + x.getMessage());
//                x.printStackTrace();
//            }
        }
//        if (DEBUG && debuglevel > 0) debug("Self-validate OK? " + ok);
